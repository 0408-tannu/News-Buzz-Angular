...
export class NewsCardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Check if bookmarked (React sends { title, link })
    ...
    // Check if liked (React sends { title })
    ...
    // Get comment count (React sends { articleURL })
    ...
    // Check mute status if showMute is enabled
    if (this.showMute) {
      const baseURL = this.getBaseURL();
      if (baseURL) {
        this.api.post<any>('/api/mute/get', { baseURL }).subscribe(res => {
          if (res?.success) this.muted = res.isMuted;
        });
      }
    }
  }
  ...
  toggleMute(event: Event): void {
    ...
    this.api.post<any>(endpoint, { baseURL }).subscribe(res => {
